...
* **gridfs:** make a copy of chunk before post to server ([b4ec5b8](https://github.com/mongodb/node-mongodb-native/commit/b4ec5b8))
...
* Updated mongodb-core to 2.1.11
    ...
    * Check for connection destroyed just before post out and flush out operations correctly if it is (Issue #179, https://github.com/jmholzinger).
...
2.0.48 2015-11-07
-----------------
* GridFS no longer performs any deletes when post a brand new file that does not have any previous <db>.fs.chunks or <db>.fs.files documents.
...
* NODE-426 ensure drain event is emitted after write function returns, fixes intermittent issues in post files to gridstore.
